...
const UsageReports = () => {
    ...
    // Filter logic (Plate Number ya Driver Name se search)
    const filteredHistory = history.filter(item => 
        item.vehicleId?.plateNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="container mt-4 pb-5">
        <div className="reports-container p-4">
            <div className="d-flex justify-content-between align-items-center mb-4">
                <h3 className="text-white"><FaHistory className="me-2 text-info" /> Usage Logs</h3>
                ...
                <table className="table custom-table">
                    ...
                    <tbody>
                        {filteredHistory.map((trip) => (
                            <tr key={trip._id}>
                                <td>
                                    <div className="fw-bold text-info">{trip.vehicleId?.plateNumber}</div>
                                    <small className="text-white-50">{trip.vehicleId?.model}</small>
                                </td>
                                <td>{trip.driverId?.name}</td>
                                <td>{new Date(trip.startTime).toLocaleString('en-GB', { hour12: true })}</td>
                                <td>{trip.endTime ? new Date(trip.endTime).toLocaleString('en-GB', { hour12: true }) : "---"}</td>
                                <td>
                                    <span className={`status-pill ${trip.status === 'Completed' ? 'status-completed' : 'status-active'}`}>
                                        {trip.status}
                                    </span>
                                </td>
                            </tr>
